...
# Team ID:			270
# Author List:		[Amirul Haqe]
# Filename:			task_1a_part1.py
# Functions:		scan_image
# 					[getColorName,side,diag,slope,mid]
...
## modules for this task (numpy, opencv2, os)                ##
##############################################################
import cv2
import numpy as np
import os
...
def scan_image(img_file_path):
    ...
    ##############	ADD YOUR CODE HERE	##############
    shapes = {}
    #reading image file, converting to gray scale, applying threshold, and finding contours
    imggray = img_file_path
    ret, thresh = cv2.threshold(imggray, 150, 255, 0)
    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #first contour is deleted as it belongs the overall boundary of the image
    del contours[0]
    ...
    for cnt in contours:
        ...
        if len(approx) >= 6:
            shape = "Circle"
        #finding centroid and area using moments
        M = cv2.moments(cnt)
        cx = int(round(M["m10"]/M["m00"],2))
        cy = int(round(M["m01"]/M["m00"],2))
        #fetching rgb values of centroid
        cname = None
        shapes[shape] = [cname,cx,cy]
